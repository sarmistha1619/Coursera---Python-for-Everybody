...
hour to test the program (the pay should be 498.75). You should use
...
def computepay(hour,hourRate):
    if (hour <= 40):
        TotalPay = hour * hourRate
        t=format(TotalPay, ".2f")
    else:
        TotalPay = ((40 * hourRate) + ((hour - 40) * (hourRate * 1.5)))
        t=format(TotalPay, ".2f"    )
    return t

hour = float(input("How many hours?"))
hourRate = float(input("What is your rate per hour?"))
print("Pay",computepay(hour,hourRate))